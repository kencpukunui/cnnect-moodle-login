...
class auth_plugin_base {
     ...
     */
    function signup_hook_form() {
        global $CFG;
        /* Ticket number is LJA-305087
         * 08062018 (Begin) change the require library to a hook form named new_signup_form.php and reture the hook form to user
         * Please see the file of this path: /home/ken/public_html/login/new_signup_form.php to read comment documentation*/
        require_once($CFG->dirroot.'/login/new_signup_form.php');
        return new new_login_signup_form(null, null, 'post', '', array('autocomplete' => 'on'));
        /* 08062018 (End)*/
    }
 ...
 * Created by Ken Chang 05/07/2018 for Pukunui Technology
 * Add the missing fields to a user that is going to be created
 * And replace $user->name filed to be $user->email
 * @param  stdClass $user the new user object
 * @return stdClass the user filled
 * @since Moodle 3.2
 */
function signup_setup_new_user_username_as_email($user) {
    global $CFG;
